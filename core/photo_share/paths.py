...
def build_thumbnail_modes(mode_settings: dict[str, dict[str, int]] | None = None) -> dict[str, dict[str, int]]:
    ...
    for mode, settings in (mode_settings or {}).items():
        if mode in modes:
            ...
            if "queue_limit" in settings:
                modes[mode]["queue_limit"] = max(10, int(settings["queue_limit"]))
    return modes
